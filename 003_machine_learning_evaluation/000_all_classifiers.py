#!/usr/bin/env python3
"""
Evaluate all generated feature datasets with the classifiers from the
scikit-learn classifier comparison example.

Source classifier list:
...
Output:
    results/machine_learning_evaluation/all_classifiers/{DATASET}/results.csv
    results/machine_learning_evaluation/all_classifiers/{DATASET}/results.json
"""
...
CONFIG_OUTPUT_ROOT = 'results/machine_learning_evaluation/all_classifiers'
CONFIG_RAW_ROOT = 'data/raw'
CONFIG_K_FOLDS = 5
CONFIG_TEST_SIZE = 0.3
CONFIG_RANDOM_STATE = 42
CONFIG_MAX_ROWS = 0
CONFIG_MAX_GAUSSIAN_PROCESS_TRAIN_ROWS = 2000
CONFIG_MAX_RBF_SVM_TRAIN_ROWS = 10000
CONFIG_MAX_QDA_FEATURES = 2000
...
def parse_args() -> argparse.Namespace:
    ...
    return argparse.Namespace(
        ...
        max_gaussian_process_train_rows=CONFIG_MAX_GAUSSIAN_PROCESS_TRAIN_ROWS,
...
def classifier_factories(random_state: int):
    return [
        ("Nearest Neighbors", lambda: KNeighborsClassifier(3)),
        ("Linear SVM", lambda: SVC(kernel="linear", C=0.025, random_state=random_state)),
        ("RBF SVM", lambda: SVC(gamma=2, C=1, random_state=random_state)),
        (
            "Gaussian Process",
            lambda: GaussianProcessClassifier(1.0 * RBF(1.0), random_state=random_state),
        ),
        ...
        ("Neural Net", lambda: MLPClassifier(alpha=1, max_iter=1000, random_state=random_state)),
...
def needs_dense(classifier_name: str) -> bool:
    return classifier_name in {"Gaussian Process", "Naive Bayes", "QDA"}
...
def classifier_skip_reason(feature_dataset: FeatureDataset, classifier_name: str, args: argparse.Namespace) -> str | None:
    n_train = estimated_train_rows(len(feature_dataset.target), args.test_size)
    n_features = int(feature_dataset.X.shape[1])
    if (
        classifier_name == "Gaussian Process"
        and args.max_gaussian_process_train_rows > 0
        and n_train > args.max_gaussian_process_train_rows
    ):
        return (
            "Gaussian Process skipped because the estimated train size "
            f"({n_train}) exceeds --max-gaussian-process-train-rows="
            f"{args.max_gaussian_process_train_rows}. This classifier scales cubically "
            "with the number of samples."
        )
    if (
        classifier_name == "RBF SVM"
        and args.max_rbf_svm_train_rows > 0
        and n_train > args.max_rbf_svm_train_rows
    ):
        return (
            "RBF SVM skipped because the estimated train size "
            f"({n_train}) exceeds --max-rbf-svm-train-rows={args.max_rbf_svm_train_rows}."
        )
    if classifier_name == "QDA" and args.max_qda_features > 0 and n_features > args.max_qda_features:
        return (
            "QDA skipped because the feature count "
            f"({n_features}) exceeds --max-qda-features={args.max_qda_features}."
        )
    return None
...
def evaluate_classifier(feature_dataset: FeatureDataset, classifier_name: str, classifier, test_size: float, random_state: int) -> dict:
    ...
    model = make_pipeline(StandardScaler(with_mean=not sparse.issparse(X_train)), classifier)
...
def skipped_result(feature_dataset: FeatureDataset, classifier_name: str, reason: str, test_size: float) -> dict:
    n_samples = int(len(feature_dataset.target))
    return {
        "feature_stage": feature_dataset.feature_stage,
        "feature_selection": feature_dataset.feature_selection,
        ...
        "classifier": classifier_name,
        "n_samples": n_samples,
        "n_train": estimated_train_rows(n_samples, test_size),
        "n_test": None,
        "n_features": int(feature_dataset.X.shape[1]),
        "balanced_accuracy": None,
        "macro_f1": None,
...
def main() -> None:
    ...
    run_config = base_config(
        args,
        exclude={
            "max_gaussian_process_train_rows",
            "max_rbf_svm_train_rows",
            "max_qda_features",
        },
    )
    ...
    for feature_dataset in feature_datasets:
        ...
        for classifier_name, factory in factories:
            for campaign_id, indices in campaigns:
                ...
                if skip_reason:
                    result = skipped_result(campaign_dataset, classifier_name, skip_reason, args.test_size)
                    result["campaign"] = campaign_id
                    result["campaign_id"] = campaign_id
                    result["evaluation_scope"] = "campaign"
    ...
    save_results(output_path, results, normalize_for_csv)
    print(f"Saved all-classifier evaluation results to {output_path}")
